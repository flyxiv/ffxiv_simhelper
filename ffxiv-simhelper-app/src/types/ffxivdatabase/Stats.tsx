...
export function getStatNames(jobAbbrev: string, gcdName: string) {
  switch (jobAbbrev) {
    ...
    case AST_EN_NAME:
    case SGE_EN_NAME:
      return [WD_STAT_EN_NAME, MIND_STAT_EN_NAME, CRIT_STAT_EN_NAME, DH_STAT_EN_NAME, DET_STAT_EN_NAME, SPS_STAT_EN_NAME, PIE_STAT_EN_NAME, gcdName];
    case DRG_EN_NAME:
    case MNK_EN_NAME:
    case SAM_EN_NAME:
    case RPR_EN_NAME:
...
export function convertToSubStatInfos(totalStats: PlayerPower, LANGUAGE_TEXTS: TextDictionary) {
  ...
  if (totalStats.piety > 0) {
    subStats.push({ name: LANGUAGE_TEXTS.PIE_STAT_NAME, value: totalStats.piety });
  }
